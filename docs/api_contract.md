...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `shape_divergence_penalty` | int | 20 | 形态 - 趋势背离扣分数 |

**使用说明：**
当 Pinbar 形态方向与 EMA60 趋势方向相反时，从总分中扣除该分数。

---

## 附录 D: 信号质量分级说明 (v2.0 新增)

### D.1 分级概述

系统采用**评分累积模式**替代原有的**级联拒绝模式**，将信号分为三个质量等级：

| 等级 | 名称 | 推送策略 | 前端展示 |
|------|------|----------|----------|
| A | 精品信号 | 推送 + 🌟高亮标记 | 金色/橙色高亮，可置顶 |
| B | 普通信号 | 正常推送 | 标准样式 |
| C | 观察信号 | 仅记录不推送 | 灰色样式或隐藏 |
...
| `quality_tier` | string | 信号质量分级：`"A"` (精品) / `"B"` (普通) / `"C"` (观察) |
| `score_details.risk_penalty` | float | 风险扣分数 (0-100) |
| `score_details.quality_tier` | string | 信号质量分级标记 |

**`is_contrarian` 字段说明：**
- `false`: 顺大势信号，价格方向与 MTF 高级别趋势一致
- `true`: 逆大势信号，评分已扣除 15 分，前端可考虑用特殊样式标记

**`quality_tier` 信号质量分级说明：**

| 等级 | 说明 | 推送行为 | 前端建议 |
|------|------|----------|----------|
| `"A"` | 精品信号 - 各项指标优秀 | 推送 + 高亮标记 | 金色/橙色高亮，置顶显示 |
| `"B"` | 普通信号 - 达到基准要求 | 正常推送 | 正常显示 |
| `"C"` | 观察信号 - 边缘但可记录 | 仅记录不推送 | 灰色显示或仅在历史记录可见 |

**分级标准参考：**
- **A 级**: 实体≤20%, 形态分≥70, 趋势分≥60, 波动分≥60, 总分≥70, 无风险扣分，非逆势
- **B 级**: 实体≤30%, 形态分≥50, 趋势分≥40, 波动分≥40, 风险扣分≤10, 非逆势
- **C 级**: 实体≤40%, 形态分≥30, 总分≥30, 风险扣分≤20
...
后端信号检测逻辑已从**级联拒绝模式**改为**评分累积模式**，主要变更：

| 过滤层 | 原行为 | 新行为 |
|--------|--------|--------|
| 形态识别 | 实体>25% 拒绝 | 实体>50% 才拒绝 |
| 趋势过滤 | Hard 模式直接拒绝 | 全部 soft 模式，只扣分 |
| 波动率 | <1.2×ATR 拒绝 | 低于阈值评分×0.3 |
| 止损风控 | 超阈值拒绝 | 超阈值扣分，2 倍才拒绝 |

### E.2 对前端的影响

1. **信号数量可能增加** - 边缘信号不再被直接过滤，建议前端增加质量分级筛选器
2. **新增质量分级显示** - 建议在信号列表中添加 `quality_tier` 列或标记
3. **C 级信号不推送** - 用户可能在后端看到 C 级信号记录但收不到推送，属于正常行为

### E.3 前端适配建议

**信号列表新增列：**
- `quality_tier` - 信号等级，建议用 Badge 组件显示 (A 级橙色、B 级蓝色、C 级灰色)
- `score_details.risk_penalty` - 风险扣分，可帮助用户理解信号质量

**筛选器新增选项：**
- 信号等级筛选：只显示 A 级 / A+B 级 / 全部
