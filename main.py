...
def assemble_engine() -> CryptoRadarEngine:
    ...
    # 实例化推流发送阵列与组合广播器
    feishu = FeishuNotifier(webhook_url="https://open.feishu.cn/open-apis/bot/v2/hook/xxx")
    telegram = TelegramNotifier(bot_token="your_tg_token", chat_id="your_group_id")
        ...
        try:
            ...
            if isinstance(intervals, list) and len(intervals) > 0:
                ...
                for k, v in intervals.items():
                    parsed_intervals[k] = IntervalConfig(**v) if isinstance(v, dict) else IntervalConfig()
                engine.monitor_intervals = parsed_intervals
        except Exception as e:
            logging.error(f"无法解析数据中的 monitor_intervals 配置: {e}")
            from core.entities import IntervalConfig
            engine.monitor_intervals = {"15m": IntervalConfig(), "1h": IntervalConfig(), "4h": IntervalConfig()}
    else:
        from core.entities import IntervalConfig
        engine.monitor_intervals = {"15m": IntervalConfig(), "1h": IntervalConfig(), "4h": IntervalConfig()}
