...
async def run_history_test():
    ...
    mock_account = AccountBalance(total_wallet_balance=10000.0, available_balance=10000.0, current_positions_count=0, positions=[])
    
    # 获取真实 Webhook，并绕过全局开关强制装载
    broadcaster = NotificationBroadcaster()
    # 根据用户要求，本次运行不需要推送企微和飞书
    # wecom_url = await repo.get_secret("wecom_webhook_url")
    # feishu_url = await repo.get_secret("feishu_secret")
    # 
    # if wecom_url:
    #     broadcaster.register(ForceNotifyWeCom(wecom_url))
    # if feishu_url:
    #     broadcaster.register(ForceNotifyFeishu(feishu_url))
    ...
    import json
    interval_configs = {"15m": IntervalConfig(), "1h": IntervalConfig(), "4h": IntervalConfig()}
    ...
    for ivl, bars in all_bars_by_interval.items():
        ...
        for current_idx in range(60, len(bars)):
            ...
            if use_mtf and higher_ivl in all_bars_by_interval:
                ...
                valid_h_bars = [b for b in h_bars if b.timestamp <= current_bar.timestamp]
            ...
            if signal:
                ...
                logger.info(f"历史发现: {timestamp_str} | 级别: {signal.interval} | 方向: {signal.direction} | 价格: {signal.entry_price}")
    ...
    if signals_found:
        ...
        for s in signals_found:
            ts = datetime.fromtimestamp(s.timestamp / 1000).strftime('%Y-%m-%d %H:%M:%S')
            print(f"- [{ts}] {s.interval}: {s.direction} @ {s.entry_price} (Score: {s.score})")
        print(f"历史信号平均质量得分: {avg_score:.1f} / 100")
        print("\n提示：这些信号已存入 SQLite，去您的 Dashboard 就可以看到了。")
    else:
        print("总计检测 15m, 1h 级别数据")
        print("在严格风控(<=3.5%)下，最近几天内未识别出满足当前策略的有效 Pinbar 信号。")
    print("="*50 + "\n")
