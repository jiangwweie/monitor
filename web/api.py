...
async def get_account_dashboard(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch account data from Binance: {e.response.text}")
...
async def get_position_detail(request: Request, symbol: str):
    ...
    if not reader:
        raise HTTPException(status_code=503, detail="Trading engine or account reader is not configured or offline.")
    ...
    try:
        ...
        if status_code in [401, 403]:
            try:
                ...
                raw_msg = e.response.text
            raise HTTPException(status_code=status_code, detail=f"BINANCE API ERROR: ({raw_msg})")
        raise HTTPException(status_code=500, detail=f"Failed to fetch position data: {e.response.text}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
class ConfigUpdateReq(BaseModel):
    ...
    auto_order_status: Optional[str] = None # 仅仅是为了接口兼容，后端将强行忽略该值的修改
...
async def get_config(request: Request):
    ...
    global_push_enabled = global_push_enabled_val.lower() == "true" if global_push_enabled_val else True # 默认开启
    
    import dataclasses
    return {
        "system_enabled": engine.system_enabled,
        "active_symbols": engine.active_symbols,
        "monitor_intervals": {k: dataclasses.asdict(v) for k, v in engine.monitor_intervals.items()} if engine.monitor_intervals else {},
        "risk_config": {
        ...
        "pinbar_config": dataclasses.asdict(engine.pinbar_config) if engine.pinbar_config else {},
...
async def update_config(request: Request, req: ConfigUpdateReq = Body(...)):
    ...
    if req.monitor_intervals is not None:
        new_intervals = {k: IntervalConfig(use_trend_filter=v.use_trend_filter) for k, v in req.monitor_intervals.items()}
        engine.monitor_intervals = new_intervals
        await repo.set_secret("monitor_intervals", json.dumps({k: dataclasses.asdict(v) for k, v in new_intervals.items()}))
    ...
    if req.pinbar_config:
        ...
        await repo.set_secret("pinbar_config", json.dumps(dataclasses.asdict(engine.pinbar_config)))
    ...
    if req.exchange_settings:
        if req.exchange_settings.binance_api_key:
            ...
            await repo.set_secret("binance_api_key", req.exchange_settings.binance_api_key)
        if req.exchange_settings.binance_api_secret:
            engine.account_reader.api_secret = req.exchange_settings.binance_api_secret
            await repo.set_secret("binance_api_secret", req.exchange_settings.binance_api_secret)

    # 安全防线：忽视所有对 auto_order_status 的修改，保持后端处于零执行的只读模式
    if req.auto_order_status is not None:
        logger.warning(f"检测到试图修改 auto_order_status 为 {req.auto_order_status}，后端安全锁已拒绝该操作。")

    # 涉及到数据库加密保存的落盘动作
    if req.webhook_settings:
        if req.webhook_settings.global_push_enabled is not None:
            await repo.set_secret("global_push_enabled", str(req.webhook_settings.global_push_enabled).lower())
            
        if req.webhook_settings.feishu_secret:
            await repo.set_secret("feishu_secret", req.webhook_settings.feishu_secret)
        
        if req.webhook_settings.wecom_enabled is not None:
            await repo.set_secret("wecom_enabled", str(req.webhook_settings.wecom_enabled).lower())
            
        if req.webhook_settings.wecom_secret:
            await repo.set_secret("wecom_secret", req.webhook_settings.wecom_secret)
            await repo.set_secret("wecom_webhook_url", req.webhook_settings.wecom_webhook_url)
