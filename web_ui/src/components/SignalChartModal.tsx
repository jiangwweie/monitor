...
export function SignalChartModal({ signal, open, onOpenChange }: SignalChartModalProps) {
    ...
    const initChart = useCallback(async () => {
        ...
        try {
            ...
            // 前后各 20 根 K 线
            const barsOnEachSide = 20;
            const totalBars = barsOnEachSide * 2 + 1; // +1 包含信号所在 K 线

            // 计算 end_time = 信号时间 + 20 根 K 线，这样 limit=41 时可以覆盖 [信号时间 -20 根，信号时间 +20 根]
            const endTime = signal.timestamp + (barsOnEachSide * ivlMs);
            const limit = totalBars;
            ...
            // 只显示当前选中的信号标记（过滤掉其他信号）
            const signalTimeSec = Math.floor(signal.timestamp / 1000);
            markers = markers.filter((m: any) => {
                // 精准匹配信号时间（允许 1 根 K 线的误差）
                return Math.abs(signalTimeSec - m.time) < (ivlMs / 1000);
            });
            ...
            // 自适应缩放 — 聚焦到信号所在的时间区域
            const rangeFrom = signalTimeSec - (barsOnEachSide * ivlMs) / 1000;
            const rangeTo = signalTimeSec + (barsOnEachSide * ivlMs) / 1000;
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-5xl bg-white/95 dark:bg-zinc-900/95 backdrop-blur-2xl border-zinc-200 dark:border-white/10 text-zinc-900 dark:text-zinc-100 rounded-3xl p-6 shadow-2xl">
                <DialogHeader>
                ...
                {/* 左右分栏布局：左侧 K 线图，右侧信号参数 */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 py-4 max-h-[85vh] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700">
                        ...
                        <div className="rounded-2xl overflow-hidden bg-zinc-50 dark:bg-black/30 border border-zinc-200/50 dark:border-white/5">
                            ...
                            <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-200/50 dark:border-white/5">
                                <div className="flex items-center gap-2 text-xs text-zinc-500">
                                    ...
                                    <span>{signal.symbol} · {signal.interval || "1h"} · K 线图表</span>
                                </div>
